...
void Game::player_Input()
{
    ...
    else if (flag_up == 0 && sf::Keyboard::isKeyPressed(sf::Keyboard::Up))
    {
        ...
        if (verify_Collision())
        {
            rotate();
            rotate();
            rotate(); // Reverses the rotation that caused a collision
        }
    }
    else if (flag_hardDrop == 0 && sf::Keyboard::isKeyPressed(sf::Keyboard::Space))
    { // Hardrop
        flag_hardDrop = 1;
        while (moveDown() == true)
        {
...
{
    if (!musicGame.openFromFile("musicGaming.ogg"))
    { // Replace with your music file path
        std::cout << "Erro ao carregar a música!" << std::endl;
    }
    if (!soundGameOver.openFromFile("soundGameOver.ogg"))
    { // Replace with your music file path
        std::cout << "Erro ao carregar a música!" << std::endl;
    }
}
...
void Game::run()
{
    ...
    { // Match pressed
        if (Lobby == 2)
        { // Player chose to enter match menu
            Match = windowGame.MatchWindow();
        }
        if (Match == 2)
        { // Player chose to return to lobby
    ...
    while (window->isOpen())
    {
        ...
        if (!gameOver && clockFall.getElapsedTime().asSeconds() > delay)
            moveDown();
    }
}
